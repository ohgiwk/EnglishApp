...
if (!frequencyPath || !dictionaryDir || !basicListPath || !schoolListPath) {
  throw new Error(
    'Usage: node scripts/generate-vocabulary.mjs <frequency-list> <ejdict-src-dir> <basic-list> <school-list> [output]'
  )
}
...
const inferPart = (definition, word) => {
  if (
    definition.includes('{動}') ||
    /する[,)・]/.test(definition) ||
    /^(be|have|do|can|will|may|must)$/.test(word)
  )
    return 'verb'
  if (definition.includes('{形}') || /(ful|less|ous|ive|able|ible|al|ic|ish|ary)$/.test(word))
    return 'adjective'
  if (definition.includes('{副}') || word.endsWith('ly')) return 'adverb'
  if (
    /^(a|an|the|and|or|but|if|because|as|of|to|in|on|at|for|from|with|by|about|into|over|after|before|under|between|through|during|without|against|among|i|you|he|she|it|we|they|me|him|her|us|them|my|your|his|its|our|their|this|that|these|those|who|what|which|where|when|why|how|some|any|each|every|all|both|many|much|few|more|most|other|another|no|not|so|than)$/.test(
      word
    )
  )
    return 'other'
...
const basicWords = new Set(
  readFileSync(basicListPath, 'utf8')
    .split(/\r?\n/)
    .map((word) => word.trim().toLowerCase())
)
const schoolWords = new Set(
  readFileSync(schoolListPath, 'utf8')
    .split(/\r?\n/)
    .map((word) => word.trim().toLowerCase())
)
const selected = []
