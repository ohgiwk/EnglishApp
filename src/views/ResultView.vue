...
<template>
  <section v-if="result" class="page result">
    <div class="clear">
      ...
      <div class="xp">+{{ result.choice.englishXp }} <small>ENGLISH XP</small></div>
    </div>
    <div class="reaction">
      <img :src="chapter.image" :alt="chapter.title" />
      <div>
        <b>{{ store.activeCharacter.englishName.split(' ')[0] }}</b>
        <p>“{{ result.choice.heroineResponse }}”</p>
        <small>{{ result.choice.heroineResponseJa }}</small>
      </div>
    </div>
    <div class="delta">
      <span
        ><Heart /> Affection - 愛情
        <b :class="{ minus: result.choice.affectionChange < 0 }"
          >{{ result.choice.affectionChange > 0 ? '+' : '' }}{{ result.choice.affectionChange }}</b
        ></span
      ><span
        ><ShieldCheck /> Trust - 信頼
        <b :class="{ minus: result.choice.trustChange < 0 }"
          >{{ result.choice.trustChange > 0 ? '+' : '' }}{{ result.choice.trustChange }}</b
        ></span
      >
    </div>
    ...
    <button class="secondary" @click="store.toggleReview(reviewId)">
