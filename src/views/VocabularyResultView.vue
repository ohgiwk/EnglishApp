...
const reviewWords = computed(() =>
  result.value?.reviewWordIds.map((id) => vocabularyWords.find((word) => word.id === id)).filter(Boolean) ?? []
)
  ...
  <section v-if="result" class="page vocab-result">
    <div class="result-celebration">
      ...
      <div class="result-stars">
        <Star v-for="index in 3" :key="index" :fill="result.accuracy >= index * 30 ? 'currentColor' : 'none'"/>
      </div>
    </div>

    <div class="result-emma">
      <EmmaPortrait :expression="result.accuracy >= 80 ? 'blush' : 'smile'"/>
      <div><b>Emma</b><p>“{{ comment }}”</p><small>{{ commentJa }}</small></div>
    </div>
    ...
    <div class="card result-list">
      ...
      <div v-for="word in reviewWords" :key="word!.id">
        <b>{{ word!.word }}</b><span>{{ word!.meaningJa }}</span>
      </div>
    </div>
